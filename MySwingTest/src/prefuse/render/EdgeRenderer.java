...
//        	VisualItem vi = (VisualItem)edge.getSourceNode();
     ...
     * 确定控制点使用立方（Bezier）曲线的边缘。重写此方法提供自定义曲线规范。减少对象的初始化，
     * 数组项point2d已经初始化，所以使用point2d。setlocation()方法而不是新point2d。double()更有效地设置自定义控制点。
