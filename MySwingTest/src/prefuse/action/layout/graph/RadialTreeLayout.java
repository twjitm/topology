...
public class RadialTreeLayout extends TreeLayout {
	 ...
	 * 创建一个新的radialtreelayout。默认情况下启用了布局边界的半径值的自动缩放功能。
	 * @param group
	 ...
	 * disabled by default.
	 * 创建一个使用指定的半径增量布局水平之间的新radialtreelayout。默认情况下，默认情况下禁用半径值的自动缩放。
	 * 
	 * @param group
	 *            the data group to process. This should resolve to either a
	 *            Graph or Tree instance.
	 ...
	 * Set the radius increment to use between concentric circles. Note that
	 * this value is used only if auto-scaling is disabled.
	 * 设置同心圆之间的半径增量。请注意，只有当禁用自动缩放时，才使用此值
	 * 
	 * @param inc
	...
	public void run(double frac) {
		...
		// calc relative widths and maximum tree depth相对宽度和最大深度
		
		// performs one pass over the tree执行一个通过树
		m_maxDepth = 0;
		...
		// update properties of the root node根节点的更新属性
	...
	private void calcAngularBounds(NodeItem r) {
		...
		// try to find previous parent of root,获取父情节点
		...
		// compute offset due to children's angular width计算偏移由于孩子的角宽度
	 ...
	 *  计算相关措施的角宽度的每个扩展子树。考虑到节点的直径
	 * 以改善可变大小的节点的空间分配。该方法还更新了节点的基角值，以确保节点的适当顺序。参数:
	 * ordering of nodes.
	 */
	...
	private static final double normalize(double angle) {//360度
	...
	private Iterator sortedChildren(final NodeItem n) {
		double base = 0;
		// update base angle for node ordering更新节点排序的基本角度
		...
		if (!c.isStartVisible()) {
			// use natural ordering for previously invisible nodes使用自然顺序为以前看不见的节点
			return n.children();
		}
		...
		// return iterator over sorted children排序的children返回迭代器
		return new Iterator() {
			int cur = 0;
	...
	/**
	 * Set the position of the given node, given in polar co-ordinates.设置位置的节点，在极坐标中。
	 * 
	 * @param n
	 *            the NodeItem to set the position nodeitem设置 位置
	 * @param p
	 *            the referrer parent NodeItem引荐 父节点nodeitem 
	 * @param r
	 *            the radius极坐标半径
	 * @param t
	 *            the angle theta角度
