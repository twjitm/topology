...
public abstract class Layout extends GroupAction {
     ...
     * 返回要计算布局的范围。如果边界已被显式设置，则使用该值。否则，
     * 尝试根据在这个动作的关联的可视化中发现的第一个显示的显示区域来计算边界。
     * Visualization.
     ...
     * 返回中心或根布局的布局锚。如何使用这一点（如果它是使用在所有）是依赖于特定的
     * 布局实现。如果没有显式地设置了锚定点，则使用此操作关联的可视化中发现的第一个显示的中心坐标，如果可用。
