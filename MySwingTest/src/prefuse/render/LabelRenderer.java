...
import prefuse.safein.demo.TransferData;
...
//			w = tw + size * (iw + 2 * m_horizBorder + (tw > 0 && iw > 0 ? m_imageMargin : 0));
		...
		// } //修改后的代码
		  ...
		  //(二次)修改的源代码----------------------------
        if (m_bbox==null||m_bbox.getCenterX()==0.0) {
        ...
        //	 try {
        //	 ResultSet rs = null;
          //   String hqltonode="SELECT * FROM report_result_external where device_name='"+m_text+"'"; 
           //  rs = con.prepareStatement(hqltonode).executeQuery();//查询
           //  String manage_key="";
         //    if (rs.next()) {
         //    manage_key=rs.getString("manage_key");
		//	}
          //  // System.out.println( "manage_key"+manage_key); 
           //  String sql="SELECT * FROM layout WHERE device_name="+"'"+m_text+"' and manage_key='"+manage_key+"'";
         //   List<String>device_name=new ArrayList<String>();//查询结果集合
         //   rs = con.prepareStatement(sql).executeQuery();//查询
           // String returnde_na="";
          //  while (rs.next()) {
             //System.out.println("查询数据库的数据="+returnde_na);
        //	  returnde_na= rs.getString("device_name");
		//}
             int a= (int) m_bbox.getCenterX();
              int b=(int) m_bbox.getCenterY();
        	  String position=a+","+b;//点的位置
        ...
        ///	 if ("".equals(returnde_na)) {//layout表中没有这个device_name
           // hql="INSERT INTO layout(device_name,manage_key,x,y,position) VALUES("+
        //	   "'"+m_text+"'"+","+"'"+manage_key+"'"+","+m_bbox.getCenterX()+","+m_bbox.getCenterY()+","+"'"+position+"'"+")";
		//	}else {
			//	 hql="UPDATE layout SET x="+m_bbox.getCenterX()+", y="+m_bbox.getCenterY()+" WHERE device_name="+"'"+m_text+"' AND manage_key="+"'"+manage_key+"'";
		//	}
        	// con.createStatement().execute(hql);
        	// con.close();
  ...
  //      this.setTuopo(map);
    ...
    public static boolean writeFile(String filePath, String content)
    {
        ...
        try
        {
            fos = new FileOutputStream("data/position/"+filePath+".txt");
